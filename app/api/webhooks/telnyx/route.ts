...
export async function POST(request: NextRequest) {
  try {
    ...
    if (payload.errors && payload.errors.length > 0) {
        logger.error('❌ SMS DELIVERY ERRORS DETECTED:', {
                  messageId: payload.id,
    ...
    if (payload.to && payload.to.length > 0) {
        payload.to.forEach((recipient: any, idx: number) => {
            ...
            logger.warn(`⚠️ Recipient ${idx + 1} Delivery Issue:`, {
...
async function handleMessageStatusUpdate(
  ...
  try {
    ...
    if (error) {
      logger.error('Failed to update SMS log:', { context: [error] });
    } else {
      logger.info('✅ SMS log updated successfully');
    }
  } catch (error) {
    logger.error('Error handling message status update:', { context: [error] });
  }
}
...
async function handleInboundMessage(supabase: any, payload: any) {
  try {
    ...
    logger.error('Error handling inbound message:', { context: [error] });
  }
}
