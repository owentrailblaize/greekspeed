...
export function CommentModal({ isOpen, onClose, post, onLike, onCommentAdded }: CommentModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] max-w-[95vw] h-[90vh] sm:h-[80vh] max-h-[700px] flex flex-col overflow-hidden p-0">
        {/* Fixed Header - Minimal but consistent padding */}
        <DialogHeader className="flex-shrink-0 border-b border-gray-200 px-4 py-3 sm:px-6 sm:py-4">
          <DialogTitle className="text-xl sm:text-lg font-semibold">
            Comments
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-hidden flex flex-col min-h-0">
          {/* Original Post - Consistent padding */}
          <div className="flex-shrink-0 border-b border-gray-200 px-4 py-4 sm:px-6 sm:py-5">
              ...
              <div className="flex-1 min-w-0">
                ...
                {post.image_url && (
                  <img 
                    src={post.image_url} 
                    alt="Post content" 
                    className="w-full max-h-60 sm:max-h-80 object-cover rounded-lg mb-3 sm:mb-4"
                  />
                )}

                {/* Post Actions - Reduced spacing */}
          ...
          {/* Comments Section - Balanced padding */}
          <div className="flex-1 overflow-y-auto min-h-0">
            {loading ? (
              <div className="px-4 py-6 sm:px-6 sm:py-8 text-center">
                <p className="text-gray-500 text-sm sm:text-base">Loading comments...</p>
              </div>
            ) : error ? (
              <div className="px-4 py-6 sm:px-6 sm:py-8 text-center">
                <p className="text-red-500 text-sm sm:text-base">Error loading comments: {error}</p>
              </div>
            ) : comments.length === 0 ? (
              <div className="px-4 py-6 sm:px-6 sm:py-8 text-center">
                  ...
                  <div key={comment.id}>
                    <div className="flex items-start space-x-3 sm:space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {/* Comment Actions - Reduced spacing */}
          ...
          {/* Comment Input - Consistent padding */}
          <div className="flex-shrink-0 border-t border-gray-200 px-4 py-3 sm:px-6 sm:py-4 bg-white">
            <div className="flex items-end space-x-3 sm:space-x-4">
              ...
              <div className="flex-1">
                <Textarea
                  ...
                  className="min-h-[40px] sm:min-h-[50px] max-h-[100px] sm:max-h-[120px] resize-none border-gray-300 focus:ring-navy-500 focus:border-navy-500 text-sm sm:text-base p-2 sm:p-3"
                  rows={1}
                />
              </div>
              ...
              <Button
                ...
                size="sm"
                className="bg-navy-600 hover:bg-navy-700 text-white h-10 sm:h-12 px-3 sm:px-4"
              >
                {isSubmitting ? (
